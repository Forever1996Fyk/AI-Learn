...
public class ExcelProcessServiceImpl implements FileProcessService {
    ...
    public void processDocument(KnowledgeDocumentEntity document, InputStream inputStream) {
        ...
        try {
            ...
            // 4. 检查表名是否已存在
            if (tableMetaService.checkTableExists(tableName) > 0) {
            ...
            // 7. 执行建表
            tableMetaService.executeCreateTable(createTableSql);
            ...
            log.info("成功插入 {} 条数据到表 {}", insertedCount, tableName);

            // 9. 保存表元数据
            TableMetaEntity tableMeta = new TableMetaEntity();
            tableMeta.setTableName(tableName);
            tableMeta.setDescription(document.getDescription() != null ? document.getDescription() : "从Excel导入: " + docTitle);
            ...
            boolean result = tableMetaService.save(tableMeta);
            Assert.isTrue(result, "保存表元数据失败");
    ...
    private int insertData(String tableName, List<ColumnInfo> columns, List<List<String>> dataRows) {
        ...
        for (int i = 0; i < dataRows.size(); i+=batchSize) {
            ...
            // 使用 JdbcTemplate 执行，绕过 MyBatis-Plus的 BlockAttackInnerInterceptor(攻击 SQL 阻断解析器,防止全表更新与删除)
    ...
    private String generateCreateTableSql(String tableName, String description, List<ColumnInfo> columns) {
        ...
        sql.append(") ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='").append(description).append("'");
    ...
    private List<ColumnInfo> generateColumnInfo(List<String> headers) {
        ...
        for (int i = 0; i < headers.size(); i++) {
            ...
            int suffix = 1;
            while (usernames.contains(columnName)) {
                columnName = originalName + "_" + suffix++;
            }
            usernames.add(columnName);
            ColumnInfo columnInfo = new ColumnInfo();
            columnInfo.setIndex(i);
            columnInfo.setOriginalHeader(header);
            columnInfo.setColumnName(columnName);
            // 默认使用VARCHAR类型
            columnInfo.setDataType("VARCHAR(500)");
    ...
    private void dropTable(String tableName) {
        ...
        transactionTemplate.executeWithoutResult(status -> {
            // 1. 删除物理表
            tableMetaService.dropTable(tableName);
            log.info("物理表{}删除成功", tableName);
            // 2. 删除元数据记录
            tableMetaService.remove(
                    new QueryWrapper<TableMetaEntity>().lambda()
    ...
    private String generateTableName(String originalTableName) {
        String baseName = originalTableName;
        // 去掉扩展名
        int docIndex = baseName.lastIndexOf('.');
        if (docIndex > 0) {
            baseName = baseName.substring(0, docIndex);
        }
        // 清理非法字符
