...
import com.forever1996Fyk.ai.agent.tools.StockTools;
...
public class ReactAgentController {
    ...
    public String chatWithSpringAiAlibaba(String conversationId) throws GraphRunnerException {
        ...
        AssistantMessage response = agent.call("帮我分析最近三个月特斯拉（TSLA）的股价走势，并结合新闻事件解释可能的影响因素。", config);
    ...
    public String callWithMcpAndAgent(String conversationId, String city) throws GraphRunnerException {
        // 配置自定义 HttpClient，增加超时时间和错误处理
        HttpClientStreamableHttpTransport streamableTransport = HttpClientStreamableHttpTransport
                .builder("http://127.0.0.1:8082/stream/test/")
                .endpoint("api/mcp")
    ...
    @GetMapping("/streamCallWithMcpAndAgent")
    public Mono<String> streamCallWithMcpAndAgent(String conversationId, String city)  {
    ...
    @GetMapping("/callWithMysqlServer")
    public String callWithMysqlServer(String conversationId, String msg) throws GraphRunnerException {
        String systemPrompt = "你是一个智能助手，请根据用户问题进行回答。";
        ...
        RunnableConfig config = RunnableConfig.builder()
                .threadId(conversationId)
                .build();
        AssistantMessage response = agent.call(msg, config);
        return response.getText();
    }
}
