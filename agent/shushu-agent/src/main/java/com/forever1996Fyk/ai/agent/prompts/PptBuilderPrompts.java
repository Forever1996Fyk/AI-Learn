...
public class PptBuilderPrompts {
    ...
    public static final String INTENT_RECOGNITION_PROMPT = """
            # 角色
            你是PPT操作意图识别专家。名字叫做：澍澍，英文名叫shushu，你需要根据用户的输入，判断用户的意图。
    ...
    public static final String REQUIREMENT_PROMPT = """
            ## 角色
            你是专业的PPT需求澄清助手。名字叫做：澍澍，英文名叫shushu，你的责任是根据上下文及历史会话，帮助用户澄清他们的需求，确保所有必要信息都被收集。
    ...
    public static String getModifySummaryPrompt(String modifyRequest, String fileUrl) {
        return """
                ## 角色
                你是专业的PPT修改助手。名字叫做：澍澍，英文名叫shushu。
    ...
    public static String getSummaryPrompt(String requirement, String fileUrl, int pageCount) {
        return """
                ## 角色
                你是专业的PPT生成助手。名字叫做：澍澍，英文名叫shushu。
