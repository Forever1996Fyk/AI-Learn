...
import com.forever1996Fyk.ai.agent.agent.SimpleReactAgent;
import com.forever1996Fyk.ai.agent.domain.OverAllState;
import com.forever1996Fyk.ai.agent.domain.SaveQuestionRequest;
import com.forever1996Fyk.ai.agent.domain.SimpleReactResult;
import com.forever1996Fyk.ai.agent.domain.UpdateAnswerRequest;
import com.forever1996Fyk.ai.agent.domain.record.CritiqueResult;
import com.forever1996Fyk.ai.agent.domain.record.PlanTask;
import com.forever1996Fyk.ai.agent.domain.record.SearchResult;
import com.forever1996Fyk.ai.agent.domain.record.TaskResult;
import com.forever1996Fyk.ai.agent.manager.AgentTaskManager;
import com.forever1996Fyk.ai.agent.prompts.PlanExecutePrompts;
...
public class PlanExecuteAgent extends BaseAgent {
    ...
    public Flux<String> callInternal(String conversationId, String question) {
        ...
        // 执行循环任务
        Runnable executeLoopPhase = () -> executeLoopPhase(state, sink, finished, finalAnswerBuffer, thinkingBuffer);

        // 研究主题生成任务
        Runnable generateResearchTopicPhase = () -> generateResearchTopicPhase(state, sink, finished, thinkingBuffer, executeLoopPhase);

        // 启动流程：需求澄清 -> 研究主题生成 -> 执行循环
        clarifyRequirementPhase(state, sink, finished, thinkingBuffer, generateResearchTopicPhase);
    ...
    private void clarifyRequirementPhase(OverAllState state, Sinks.Many<String> sink,
        ...
        Disposable disposable = chatClient.prompt()
                ...
                .doOnComplete(() -> handleClarificationComplete(responseBuffer, sink, finished, thinkingBuffer, onComplete))
    ...
    private void generateResearchTopicPhase(OverAllState state, Sinks.Many<String> sink,
        ...
        Disposable disposable = chatClient.prompt()
                ...
                .doOnComplete(() -> handleResearchTopicComplete(topicBuffer, state, sink, finished, thinkingBuffer, onComplete))
        ...
        return Mono.fromRunnable(() -> {
            try {
                ...
                while (state.getRound() < maxRounds && !finished.get() && !compositeDisposable.isDisposed()) {
                    ...
                    // 生成计划
                    List<PlanTask> planTasks = generatePlan(state, sink, finished, thinkingBuffer);
                    if (finished.get() || compositeDisposable.isDisposed()) {
                        return;
                    }
                    if (CollectionUtils.isEmpty(planTasks) || planTasks.stream().allMatch(t -> StringUtils.isBlank(t.id()))) {
                        break;
                    }
                    ...
                    Map<String, TaskResult> results = executePlan(planTasks, state, sink, finished, thinkingBuffer);
                    if (finished.get() || compositeDisposable.isDisposed()) {
                        return;
                    }
                    // 执行完成后的分隔
                    ...
                    // 执行一次批判
                    CritiqueResult critique = critique(state, planTasks, results, sink, finished, thinkingBuffer);
                    if (finished.get() || compositeDisposable.isDisposed()) {
                        return;
                    }
    ...
    private CritiqueResult critique(OverAllState state, List<PlanTask> currentPlan,
        ...
        if (MapUtils.isNotEmpty(currentResults)) {
            for (Map.Entry<String, TaskResult> entry : currentResults.entrySet()) {
                TaskResult result = entry.getValue();
                if (Objects.nonNull(result) && result.success() && StringUtils.isNotBlank(result.output())) {
                    userMessage.append(String.format("任务 %s: %s\n\n",
                            entry.getKey(), result.output()));
                } else if (Objects.nonNull(result) && !result.success() && StringUtils.isNotBlank(result.error())) {
        ...
        String systemMessage = PlanExecutePrompts.CRITIQUE + "\n" + converter.getFormat();
     ...
     * @param state             整体状态
     * @param sink              响应流
     * @param finished          是否已发送最终结果
     * @param finalAnswerBuffer 最终答案缓冲
     * @param thinkingBuffer    思考过程缓冲
     */
    private void summarizeStream(OverAllState state,
        ...
        Prompt prompt = new Prompt(List.of(
                ...
                new UserMessage("""
                        ...
                        StringUtils.defaultIfBlank(toolResults, "（未检索到相关结果）")
                ))
        ));
    ...
    private Map<String, TaskResult> executePlan(List<PlanTask> plan, OverAllState state, Sinks.Many<String> sink,
        ...
        Map<String, TaskResult> results = Maps.newConcurrentMap();
        ...
        TreeSet<Integer> orders = Sets.newTreeSet(grouped.keySet());
        for (Integer order : orders) {
            ...
            List<PlanTask> planTasks = grouped.get(order);

            // 使用CountDownLatch等待当前order组全部完成
            CountDownLatch latch = new CountDownLatch(planTasks.size());
            for (PlanTask planTask : planTasks) {
                ...
                Disposable disposable = Mono.fromRunnable(() -> {
                            ...
                            try {
                                ...
                                acquired = true;
                                if (planTask == null || StringUtils.isBlank(planTask.id())) {
                                    return;
                                }
                                ...
                                TaskResult result = executeWithRetry(planTask, dependencyContext, sink, hasSentFinal, thinkingBuffer);
                                results.put(planTask.id(), result);
                                if (result.success() && StringUtils.isNotBlank(result.output())) {
                                    accumulatedResults.put(planTask.id(), result.output());
                                }

                                // 构建任务结果消息，只在有错误时才显示 error
                                StringBuilder resultMessage = new StringBuilder();
                                resultMessage.append("【Completed Task Result】\n");
                                resultMessage.append("taskId: ").append(planTask.id()).append("\n");
                                ...
                                log.info("Task {} 执行被中断", planTask.id());
                                Thread.currentThread().interrupt();

                                results.put(planTask.id(),
                                        new TaskResult(
                                                planTask.id(),
                                                false,
                                                null,
                                ...
                                if (compositeDisposable.isDisposed() || Thread.currentThread().isInterrupted()
                                        || (e.getMessage() != null && e.getMessage().contains("interrupted"))) {
                                    log.info("Task {} 执行被用户停止: {}", planTask.id(), e.getMessage());
                                    results.put(planTask.id(),
                                            new TaskResult(
                                                    planTask.id(),
                                                    false,
    ...
    private TaskResult executeWithRetry(PlanTask task, String dependencyContext,
        ...
        try {
            ...
            // 收集搜索结果到allReference
            if (CollectionUtils.isNotEmpty(result.getSearchResults())) {
        ...
        // 执行失败
        emit(sink, hasSentFinal, "\n❌ 任务 " + task.id() + " 执行失败: " + lastError.getMessage() + "\n\n", "thinking", thinkingBuffer);
    ...
    private String buildDependencyContext(Map<String, String> results, List<PlanTask> plan, Integer currentOrder) {
        ...
        for (Map.Entry<String, String> entry : results.entrySet()) {
            // 查找任务对应的order
            PlanTask planTask = plan.stream()
                    .filter(task -> StringUtils.isNotBlank(task.id()) && task.id().equalsIgnoreCase(entry.getKey()))
                    .findFirst()
                    .orElse(null);

            if (planTask != null && planTask.order() == currentOrder - 1) {
                // 只有上一个order的结果才是依赖
                if (!hasDependencies) {
                    context.append("任务");
    ...
    private List<PlanTask> generatePlan(OverAllState state, Sinks.Many<String> sink, AtomicBoolean hasSentFinal, StringBuilder thinkingBuffer) {
        ...
        String systemMessage = String.format(PlanExecutePrompts.getCurrentTime() + "\n\n" + PlanExecutePrompts.PLAN +
                """ 
                         ## 当前上下文
                         当前轮次: %s
                         ...
                         ## 输出格式
                         %s
                        """, state.getRound(), toolDesc, converter.getFormat());
        String userMessage = String.format("""
                【研究主题】
                 ...
                 2. 新的计划必须直接解决这些问题
                 3. 不要重复之前失败的尝试
                 """, StringUtils.defaultIfBlank(state.getRefinedResearchTopic(), state.getQuestion()), state.renderFullContext());
    ...
    private OverAllState initStateAndSaveQuestion(String conversationId, String question) {
        ...
        // 保存用户问题到数据库
        if (StringUtils.isNotBlank(conversationId) && sessionService != null) {
    ...
    private void saveSessionResult(String conversationId, StringBuilder finalAnswerBuffer, StringBuilder thinkingBuffer) {
        ...
        try {
            // 检查是否有任何内容需要保存
            boolean hasContent = !finalAnswerBuffer.isEmpty() || !thinkingBuffer.isEmpty();
