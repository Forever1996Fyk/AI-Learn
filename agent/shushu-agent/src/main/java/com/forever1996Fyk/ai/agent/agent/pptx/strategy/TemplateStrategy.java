...
import com.forever1996Fyk.ai.agent.domain.TemplateSelectionResult;
...
public class TemplateStrategy implements PptStateStrategy {
    ...
    public void execute(AiPptInstEntity inst, Sinks.Many<String> sink, String query, StringBuilder thinkingBuffer, PptStateStrategyContext context) {
        ...
        // 获取所有可用模板
        List<AiPptTemplateEntity> templates = context.getPptTemplateService().getAllTemplates();
        // 构建模板信息字符串
        StringBuilder templatesInfo = new StringBuilder();
        for (AiPptTemplateEntity template : templates) {
        ...
        try {
            ...
            log.info("模板选择结果: templateCode={}, reason={}", result.templateCode(), result.reason());

            context.getPptInstService().updateTemplateCode(inst.getId(), result.templateCode(), TARGET_STATUS);
