...
/**
 * @program: AI-Learn
 * @description: Agent任务管理器 / 用于管理流式输出的停止和中断
 * @author: YuKai Fan
...
public class AgentTaskManager implements InitializingBean, DisposableBean {
     ...
     * 仅包含当前实例上运行的任务
     */
    private final Map<String, TaskInfo> taskMap = Maps.newConcurrentMap();
    ...
    public boolean stopTask(String conversationId) {
        ...
        // 2. 先检查redis中是否存在该任务，不存在则无需广播
        RBucket<String> bucket = getTaskBucket(conversationId);
        if (!bucket.isExists()) {
            return false;
        }

        // 3. c持有者是本实例, 说明已在处理中，无需广播
        String holder = bucket.get();
        if (instanceId.equals(holder)) {
            log.debug("会话 {} 已在本地处理中，无需广播停止任务", conversationId);
            return false;
        }

        // 4. 本地没有但redis有，且持有这不是本实例，Pub/Sub广播停止请求
        long receivers = stopTopic.publish(conversationId);
        log.info("发布停止广播：conversationId={}, 订阅组数量={}", conversationId, receivers);
        return true;
    }
