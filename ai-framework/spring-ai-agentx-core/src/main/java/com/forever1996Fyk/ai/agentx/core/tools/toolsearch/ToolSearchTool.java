...
/**
 * @program: AI-Learn
 * @description: 工具搜索元工具 — 让 LLM 按需搜索并加载 deferred 工具。
 * <p>
...
public class ToolSearchTool {
     ...
     * @return ToolCallback
     */
    public static ToolCallback createWithCatalog(ToolSearchConfig config, Map<String, ToolCallback> deferredPool, List<TokenizedTool> catalog, Set<String> discoveredNames, ChatModel chatModel) {
        ...
        for (Map.Entry<String, ToolCallback> entry : deferredPool.entrySet()) {
            ...
            catalog.add(
                    new TokenizedTool(
                            ...
                            tokenizeName(name),
                            tokenizeText(name + " " + StringUtils.defaultIfBlank(description, ""))
                    )
            );
        }
    ...
    record TokenizedTool(String name, String description, List<String> nameTokens, List<String> descTokens) {
    }
    ...
    static class ToolSearchFunction implements Function<ToolSearchFunction.SearchRequest, String> {
        ...
        public ToolSearchFunction(ToolSearchConfig config, Map<String, ToolCallback> deferredPool, Set<String> discoveredNames, ChatModel chatModel, List<TokenizedTool> catalog) {
            this.config = config;
        ...
        public String apply(SearchRequest request) {
            ...
            ToolSearchConfig.Mode mode = config.mode();
            if (mode == ToolSearchConfig.Mode.KEYWORD || mode == ToolSearchConfig.Mode.HYBRID) {
                ...
                log.debug("keywordSearch found {} tools", foundNames.size());
            }
            if (mode == ToolSearchConfig.Mode.LLM || (mode == ToolSearchConfig.Mode.HYBRID && foundNames.isEmpty())) {
                List<String> llmResults = llmSearch(chatModel, catalog, query, config.maxResults());
                foundNames.addAll(llmResults);
                log.debug("LLM Search found {} tools", foundNames.size());
            }

            foundNames = foundNames.stream().distinct().limit(config.maxResults()).toList();
            if (foundNames.isEmpty()) {
                return "未找到相关工具，请尝试使用不同的关键词，或者其他方式描述你的需求。";
            }
